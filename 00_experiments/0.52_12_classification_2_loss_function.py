...
if __name__ == '__main__':
    ...
    L = - sp.ln(sp.E**(-w1 + w3) / (sp.E**(-w1 + w3) + sp.E**(-w2 + w4))) \
        - sp.ln(sp.E**(w2 + w4) / (sp.E**(w1 + w3) + sp.E**(w2 + w4)))
    ...
    for i in range(n_iterations):
        if i == 0:
            print(f'Initial values of vector a = ({a[0]}, {a[1]}, {a[2]}, {a[3]})')
        a -= h * grad(a, funcs)
        print(f'After {i+1} steps of gradient descent vector a = ({a[0]}, {a[1]}, {a[2]}, {a[3]})')
